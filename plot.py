...
def plot_Htildepi(wdir,file_name,Q2,mode=0):
    ...
    ax11.set_ylim(-0.10,0.05)
    ax12.set_ylim(-0.05,0.10)

    ax11.set_yticks([-0.08,-0.04,0.00,0.04,0.08])
    ax12.set_yticks([-0.04,0.00,0.04,0.08])
    ...
    ax11.text(0.05 ,0.75  ,r'\textrm{\textbf{fav}}'            , transform=ax11.transAxes,size=40)
    ax12.text(0.05 ,0.75  ,r'\textrm{\textbf{unf}}'            , transform=ax12.transAxes,size=40)

    ax11.set_ylabel(r'\boldmath$z\tilde{H}(z)$',size=30)
    ...
    filename = '%s/gallery/lhapdf-Htildepi-Q2=%3.5f'%(wdir,Q2)
    if mode==1: filename += '-bands'
    filename+='.png'
...
def plot_sivers(wdir,file_name,Q2,mode=0):
    ...
    ax11.set_ylim(-0.06,0.01)
    ax12.set_ylim(-0.01,0.06)

    ax11.set_yticks([-0.06,-0.04,-0.02,0])
    ax12.set_yticks([0,0.02,0.04,0.06])
    ...
    ax11.set_ylabel(r'\boldmath$xf_{1T}^{\perp(1)}(x)$',size=30)

    if Q2 == 1.27**2: ax11.text(0.05,0.08,r'$Q^2 = m_c^2$'                                  , transform=ax11.transAxes,size=20)
    else:             ax11.text(0.05,0.08,r'$Q^2 = %s$'%Q2 + ' ' + r'\textrm{GeV}' + r'$^2$', transform=ax11.transAxes,size=20)
    ...
    legend1 = ax11.legend(handles,labels,loc='lower right',fontsize=20,frameon=0,handletextpad=0.5,handlelength=0.9,ncol=1,columnspacing=1.5)


    py.tight_layout()
    py.subplots_adjust(hspace = 0, wspace = 0.20)

    filename = '%s/gallery/lhapdf-sivers-Q2=%3.5f'%(wdir,Q2)
    if mode==1: filename += '-bands'
    filename+='.png'
...
if __name__=="__main__":
    ...
    plot_Htildepi(wdir,file_name,Q2,mode=1)
